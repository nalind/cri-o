...
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (graphdriver.Driver, error) {
	...
	rootPair := idtools.NewIDMappingsFromMaps(uidMaps, gidMaps).RootPair()
	// Create the driver home dir
	if err := os.MkdirAll(path.Join(home, linkDir), 0711); err != nil && !os.IsExist(err) {
		return nil, err
	}

	supportsDType, err := supportsOverlay(home, fsMagic, rootPair.UID, rootPair.GID)
	if err != nil {
...
func (d *Driver) create(id, parent string, opts *graphdriver.CreateOpts) (retErr error) {
	...
	if err := idtools.MkdirAs(dir, 0711, rootUID, rootGID); err != nil {
		return err
	}
	...
	if err := idtools.MkdirAs(path.Join(dir, "work"), 0711, rootUID, rootGID); err != nil {
		return err
	}
	if err := idtools.MkdirAs(path.Join(dir, "merged"), 0711, rootUID, rootGID); err != nil {
		return err
	}

	// if no parent directory, create a dummy lower directory and skip writing a "lowers" file
	if parent == "" {
		return idtools.MkdirAs(path.Join(dir, "empty"), 0711, rootUID, rootGID)
	}

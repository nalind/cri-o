...
func newLayerStore(rootdir, rundir, layerdir string, driver drivers.Driver, uidMap, gidMap []idtools.IDMap) (LayerStore, error) {
...
func (r *layerStore) Mount(id, mountLabel string) (string, error) {
	...
	if mountpoint != "" && err == nil {
		...
		err = r.Save()
		if err == nil {
			if _, relErr := filepath.Rel(r.rootdir, mountpoint); relErr == nil {
				d := mountpoint
				for {
					...
					if st.Mode().Perm()&011 != 011 {
						if chmodErr := os.Chmod(d, st.Mode().Perm()|011); chmodErr != nil {
							break
						}
					}
					if d = filepath.Dir(d); d == r.rootdir {
						break
					}
				}
			}

...
func GetStore(options StoreOptions) (Store, error) {
	...
	if err := os.MkdirAll(options.GraphRoot, 0711); err != nil && !os.IsExist(err) {
		return nil, err
	}
	for _, subdir := range []string{"mounts", "tmp", options.GraphDriverName} {
		if err := os.MkdirAll(filepath.Join(options.GraphRoot, subdir), 0711); err != nil && !os.IsExist(err) {
			return nil, err
		}
	}
...
func (s *store) load() error {
	...
	if err := os.MkdirAll(gcpath, 0711); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(rcpath, 0711); err != nil {
		return err
	}
...
func (s *store) LayerStore() (LayerStore, error) {
	...
	rls, err := newLayerStore(s.graphRoot, rlpath, glpath, driver, s.uidMap, s.gidMap)
	if err != nil {
		return nil, err
	}
	s.layerStore = rls
...
func (s *store) ContainerDirectory(id string) (string, error) {
	...
	middleDir := s.graphDriverName + "-containers"
	gcpath := filepath.Join(s.GraphRoot(), middleDir, id)
	if err := os.MkdirAll(gcpath, 0711); err != nil && !os.IsExist(err) {
		return "", err
	}
	gcpath = filepath.Join(s.GraphRoot(), middleDir, id, "userdata")
	if err := os.Mkdir(gcpath, 0755); err != nil && !os.IsExist(err) {
		return "", err
	}
	return gcpath, nil
}
...
func (s *store) ContainerRunDirectory(id string) (string, error) {
	...
	middleDir := s.graphDriverName + "-containers"
	rcpath := filepath.Join(s.RunRoot(), middleDir, id)
	if err := os.MkdirAll(rcpath, 0711); err != nil {
		return "", err
	}
	rcpath = filepath.Join(s.RunRoot(), middleDir, id, "userdata")
	if err := os.Mkdir(rcpath, 0755); err != nil && !os.IsExist(err) {
		return "", err
	}
	return rcpath, nil
}
...
func (s *store) SetContainerDirectoryFile(id, file string, data []byte) error {
	...
	err = os.MkdirAll(filepath.Dir(filepath.Join(dir, file)), 0711)
	if err != nil {
		return err
	}
